    ...
    <Zoom>{/*Using Zoom Effect*/}
      <div id='mission' className='flex flex-col h-full mt-6 '>
        ...
        <div className='container grid justify-center w-auto grid-cols-1 p-10 mx-auto ml-auto mr-auto duration-500 transform place-content-center md:p-8 lg:grid-cols-2 2xl:grid-cols-3 gap-x-4 gap-y-4 '>
          {missions.map((e) => {
            return (
              <div key={e.id} className='h-full max-w-sm p-10 pb-6 mx-auto my-auto duration-300 transform shadow-md cursor-pointer hover:-translate-y-1n hover:shadow-xl'>
                ...
                <p key={e.title}
                  className="text-[16px] lg:text-[18px] w-[14rem] mb-4 mx-auto mt-2 text-center">{e.content}</p>
              </div>
            );
          })}
        </div>
